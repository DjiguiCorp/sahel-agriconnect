...
/* ─── HOME TAB ──────────────────────────────────────────────────────── */
function HomeTab({ investor, investments, t, navigate }) {
  ...
  return (
    <div className="space-y-5 pb-24">
      <div className="px-4 pt-6">
        <h1 className="font-bold text-2xl" style={{ color: '#F5F0E8' }}>
      ...
      <div
        ...
        style={{ background: '#F5F0E8', border: '2px solid rgba(181,133,10,0.35)' }}
      >
        ...
        <button
          ...
          className="text-xs font-bold text-[#B5850A] whitespace-nowrap shrink-0"
        >
      ...
      {mainInv ? (
        ...
        >
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'rgba(245,240,232,0.55)' }}>
            {t('investorPortal.home.contribution')}
          </p>
          <p className="tabular-nums text-[#B5850A] text-4xl font-bold">{formatUSD(mainInv.amountDeployed)}</p>
          <p className="text-xs mt-1" style={{ color: 'rgba(245,240,232,0.55)' }}>
            {t('investorPortal.portfolio.approximate')} {formatEUR(mainInv.amountDeployed)} ·{' '}
            {t('investorPortal.home.growingIn')} {mainInv.opportunityName || 'Africa'} 🌍
          </p>
          <div className="mt-4">
          ...
          {mainInv.payoutSchedule?.[0] && (
            <p className="text-xs mt-3" style={{ color: 'rgba(245,240,232,0.55)' }}>
              {t('investorPortal.home.nextHarvest')}:{' '}
              <span className="font-semibold" style={{ color: '#F5F0E8' }}>
                {new Date(mainInv.payoutSchedule[0].payoutDate).toLocaleDateString()}
              </span>
            </p>
          )}
          <button
            type="button"
            onClick={() => navigate('/afri-yield/opportunities')}
            className="mt-3 text-[#B5850A] text-xs hover:underline text-left"
          >
        ...
        <div
          ...
          style={{ background: '#F5F0E8', borderTop: '4px solid #1a3c2e' }}
        >
          <div className="flex justify-center gap-3 mb-4 text-3xl">🌱 → 🌿 → 🌾</div>
          <h3 className="text-[#1a3c2e] font-bold text-lg text-center mb-2">
            {t('investorPortal.onboarding.title')}
          </h3>
      ...
      <div className="mx-4 grid grid-cols-2 gap-3">
        ...
        <a
          href={`mailto:info@djiguicorporation.org?subject=Advisory Call - AfriYield&body=Name: ${encodeURIComponent(investor?.fullName || '')}%0AEmail: ${encodeURIComponent(investor?.email || '')}`}
...
function FarmTab({ investments, t, navigate }) {
  ...
  return (
    <div className="px-4 pt-6 pb-24 space-y-5">
      <div>
        <h2 className="font-bold text-xl" style={{ color: '#F5F0E8' }}>
            ...
            <div className="grid grid-cols-3 p-4 gap-2" style={{ background: '#0d1f17' }}>
              {[
                ...
                [
                  ...
                  `+${inv.expectedROIPercent ?? 8}% ${t('investorPortal.portfolio.perYear')}`,
                ],
                [
                  t('investorPortal.portfolio.payoutDate'),
                  inv.payoutSchedule?.[0]?.payoutDate
                    ? new Date(inv.payoutSchedule[0].payoutDate).toLocaleDateString(undefined, {
                        month: 'short',
                        year: 'numeric',
                      })
                    : 'Jun 2026',
              ...
              <div className="space-y-2">
                {JOURNEY_STEPS.map((step, i) => (
                  <div key={step} className="flex items-center gap-2">
                    ...
                    <p
                      className={`text-sm ${
                        i <= currentStep ? 'text-[#F5F0E8]' : 'text-white/30'
                      }`}
                    >
...
function PricesTab({ t }) {
  ...
  return (
    <div className="px-4 pt-6 pb-24 space-y-5">
      ...
      {showPremium && <PremiumModal t={t} onClose={() => setShowPremium(false)} />}
    </div>
  );
}
...
function NewsTab({ t }) {
  ...
  return (
    <div className="px-4 pt-6 pb-24 space-y-4">
      ...
      {NEWS.map(({ key, tag, date, time, premium }) => (
        <div
          key={key}
          className={`rounded-2xl overflow-hidden relative ${premium ? 'cursor-pointer' : ''}`}
          style={{ background: '#132a1e', border: '1px solid rgba(255,255,255,0.06)' }}
          onClick={premium ? () => setShowPremium(true) : undefined}
          onKeyDown={
            premium
              ? (e) => {
                  if (e.key === 'Enter' || e.key === ' ') setShowPremium(true);
                }
              : undefined
          }
          role={premium ? 'button' : undefined}
          tabIndex={premium ? 0 : undefined}
        >
          <div className="p-4">
            ...
            <p
              className={`font-semibold text-sm leading-snug ${premium ? 'line-clamp-2' : ''}`}
              style={{ color: '#F5F0E8' }}
            >
              {t(`investorPortal.news.items.${key}title`)}
            </p>
            {!premium && (
              <p className="text-xs mt-2 leading-relaxed" style={{ color: 'rgba(245,240,232,0.55)' }}>
          ...
          {premium && (
            <div
              ...
              style={{ background: 'rgba(13,31,23,0.88)', backdropFilter: 'blur(4px)' }}
            >
              <Lock className="w-5 h-5 text-[#B5850A] mb-2" />
              <p className="text-xs text-center px-2" style={{ color: 'rgba(245,240,232,0.7)' }}>
                {t('investorPortal.news.premiumUnlock')}
              </p>
              <button
                type="button"
                className="mt-2 text-xs font-bold text-[#B5850A]"
                onClick={(e) => {
                  e.stopPropagation();
                  setShowPremium(true);
                }}
...
function HelpTab({ investor, t }) {
  ...
  return (
    <div className="px-4 pt-6 pb-24 space-y-5">
      ...
      <div className="space-y-2">
        {faqs.map((q) => (
          <div
            ...
            style={{ background: '#132a1e', border: '1px solid rgba(255,255,255,0.08)' }}
          >
            <button
              type="button"
            ...
            >
              <span className="font-medium text-sm pr-2" style={{ color: '#F5F0E8' }}>
      ...
      <div
        className="rounded-2xl p-4"
        style={{ background: '#132a1e', border: '1px solid rgba(181,133,10,0.2)' }}
      >
        ...
        {sent ? (
          ...
          <>
            ...
            <button
              ...
              className="mt-2 w-full rounded-2xl py-3 font-bold text-sm"
              style={{ background: '#B5850A', color: '#0d1f17' }}
            >
          ...
          {[
            {
              ...
              href: `https://wa.me/?text=${encodeURIComponent(
                `Hello — AfriYield investor. Email: ${investor?.email || ''}`
              )}`,
            },
            {
              icon: '📧',
...
export default function InvestorPortal() {
  ...
  return (
    <div
      className="mx-auto w-full max-w-[375px] min-h-screen pb-2"
      style={{ background: '#0d1f17', fontFamily: "'DM Sans', sans-serif" }}
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&display=swap');
      `}</style>
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-[#B5850A] font-bold text-sm shrink-0">AfriYield</span>
          <span style={{ color: 'rgba(245,240,232,0.25)' }}>·</span>
          <span className="text-xs truncate" style={{ color: 'rgba(245,240,232,0.55)' }}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            aria-label={t('investorPortal.notifications.title')}
          >
          ...
          <button
            type="button"
            onClick={signOut}
            className="text-xs transition max-w-[4.5rem] truncate"
            style={{ color: 'rgba(245,240,232,0.45)' }}
          >
      ...
      {showNotifications && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setShowNotifications(false)}>
          <div
            className="w-full max-w-[375px] h-full overflow-y-auto shadow-2xl"
      ...
      <div>
        {activeTab === 'home' && (
          <HomeTab investor={investor} investments={investments} t={t} navigate={navigate} />
        )}
        {activeTab === 'farm' && <FarmTab investments={investments} t={t} navigate={navigate} />}
        {activeTab === 'prices' && <PricesTab t={t} />}
        {activeTab === 'news' && <NewsTab t={t} />}
        {activeTab === 'help' && <HelpTab investor={investor} t={t} />}
      </div>

      <div
        className="fixed bottom-0 left-1/2 -translate-x-1/2 z-40 flex w-full max-w-[375px]"
        style={{
      ...
      >
        {TABS.map(({ key, icon: Icon, labelKey }) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-5 h-5 transition ${active ? 'text-[#B5850A]' : ''}`}
                style={!active ? { color: 'rgba(245,240,232,0.35)' } : undefined}
              />
              <span
                className={`text-xs transition ${active ? 'text-[#B5850A] font-semibold' : ''}`}
                style={!active ? { color: 'rgba(245,240,232,0.35)' } : undefined}
              >
